...
export class VideoPromoScreen extends Component {
   ...
   onChange = (event, selectedDate) => {
    ...
    constructor(props){
      ...
      getBrochure = async () =>{
        ...
        setTimeout(()=> controller.abort(), global.TimeOut);
          fetch(global.Api + "/Api/GetBrochureVideo?PamCode=" + this.props.route.params.PamCode,
          {
    ...
    render() {
        return (
           ...
           <HeaderBack Name="VIDEO" navigation={this.props.navigation}/>

<View
style={{flex:1,padding:20,flexDirection:'row',flexWrap:'wrap'}}
>


{
        this.state.ListBrochure.map((item,key)=>(
<View style={{width:350,height:250,marginRight:20}} key={key}>
  <Text style={{fontSize:15,fontWeight:'bold',marginBottom:10}}>{item.Title}</Text>
<YoutubePlayer
        height={250}
        play={false}
        videoId={item.Image}
...
export default VideoPromoScreen
